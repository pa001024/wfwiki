...
function p.renderLink(frame)
    ...
    local Name = trans.toChineseCI(Link)

    return '[['..Link..'|'..Name..']]'
end

local colorTransTable = {
    ["Peculiar"] = "#aaaaaa",
    ["N/A"] = "#dca156",
    ["Common"] = "#dca156",
    ["Uncommon"] = "#cccccc",
...
function p.renderStyledLink(frame)
    local ModName = frame.args[1]
    local Mod = getMod(ModName)
    ...
    local Color = colorTransTable[Mod.Rarity]

    local renderIcon
    
    if(Mod.Polarity ~= nil) then
        renderIcon = Icon._Pol(Mod.Polarity)
    else
        renderIcon = "None"
    end

    -- return renderIcon..'[['..Link..'|<span style="color:'..Color..';text-shadow:1px 1px 1px #666666">'..Name..'</span>]]'
    -- [[File:MiniMapMod.png|x18px|link=MOD]]
    return '[[File:MiniMapMod.png|x18px|link='..Link..']][['..Link..'|'..Name..']]'
end

function p.renderStyledCard(frame)
    local ModName = frame.args[1]
    local Mod = getMod(ModName)
    ...
    if(Mod.Image ~= nil) then
        renderImage = Mod.Image
    else
        renderImage = "Panel.png"
    end
    return '[[File:'..renderImage..'|x40px|link='..Link..']][['..Link..'|'..Name..']]'
end
...
function p.getValue(frame)
    ...
    if(Mod.Link ~= nil) then
        zhName = trans.toChineseCI(Mod.Link)
    else
        zhName = trans.toChineseCI(Mod.Name)
    end
    
    if(UpName == "NAME") then
        return ModName
    elseif(UpName == "TITLE") then
        return zhName
